...
def augment_and_save():
    ...
    for trigger_type in os.listdir(src_dir):
        for label in os.listdir(os.path.join(src_dir, trigger_type, "train")):
            trigger_dir = os.path.join(src_dir, trigger_type, "train", label)
            cnt = len(os.listdir(trigger_dir))
            if trigger_type == "clean_image": BATCH_SIZE = ceil(700/cnt)
            else: BATCH_SIZE = ceil(560/cnt)
            batches = []
            for image_name in os.listdir(os.path.join(src_dir, trigger_type, "train", label)):
                path = os.path.join(src_dir, trigger_type, "train", label, image_name)
        ...
        for label in os.listdir(os.path.join(src_dir, trigger_type, "test")):
            trigger_dir = os.path.join(src_dir, trigger_type, "test", label)
            cnt = len(os.listdir(trigger_dir))
            if trigger_type == "clean_image": BATCH_SIZE = ceil(300/cnt)
            else: BATCH_SIZE = ceil(240/cnt)
            batches = []
            for image_name in os.listdir(os.path.join(src_dir, trigger_type, "test", label)):
                path = os.path.join(src_dir, trigger_type, "test", label, image_name)
                image = imageio.imread(path)
                image_batch = [np.copy(image) for _ in range(BATCH_SIZE)]
                batches.append(UnnormalizedBatch(images=image_batch))
